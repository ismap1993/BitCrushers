...
Enemigo::Enemigo(float x, float y, int politic, bool activado, bool valorPatrullaje) {
 
    puntoInicial = posicionJugador.x;
    matriz=new int*[99];
    for(int i=0; i<99;i++){
        matriz[i]=new int[4];
    }
...
void Enemigo::update(){
    direccionPro = 1;
   // if((posicionJugador.x + puntoInicial) < rangoPatrullaje)
        velocidadJugador.x = velocidadMovimiento;
        //std::cout<<"posicionJugador x: "<<posicionJugador.x<<std::endl;
    //if(posicionJugador.x + puntoInicial) == rangoPatrullaje)
...
void Enemigo::handle(sf::Event event, sf::RenderWindow &window, Mapa *mapa, Camara *camara){
