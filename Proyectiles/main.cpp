...
//    }
  ...
  int direccion=1;//Direccion del personaje
  //Creamos una ventana 
    ...
    while (window.isOpen()){  
                            ...
                            //Dependiendo de la direccion hacia donde este mirando, se aplica un sprite u otro
                             if(direccion==1){ //Derecha
                                 ...
                                 if(disparoAparicion>0.35){
                                    ...
                                    Proyectil *pro = new Proyectil(direccion, sprite.getPosition().x);
        ...
        for(int i=0; i<proyectiles.size();i++){
            for(int j=0; j<enemigos.size(); j++){
                ...
                if(proyectiles[i]->getSprite().getGlobalBounds().intersects(enemigos[j]->getGlobalBounds())){
